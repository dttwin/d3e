...
LABELS = {
    "traveltime": "Average travel time [s]",
    "density": "Traffic density [veh/km]",
    "waitingTime": "Cumulative waiting time [s]",
    "timeLoss": "Cumulative time loss [s]",
    "speed": "Speed [m/s]",
}
...
xml_file_path = '../outputs/old_edgedata.traffic.out.xml'
...
def generate_summary_graphs_by_intervals(interval_data, output_dir):
    ...
    for attribute, y_data in y_dict_sum.items():
        ...
        for ext in ['png', 'pdf', 'svg']:
            ...
            edge_output_dir = os.path.join(output_dir, ext, attribute)
            os.makedirs(edge_output_dir, exist_ok=True)
            output_file = os.path.join(edge_output_dir, f"edge_{edge_id}_attribute_{attribute}_sum.{ext}")
            plt.savefig(output_file)
    ...
    for attribute, y_data in y_dict_mean.items():
        ...
        for ext in ['png', 'pdf', 'svg']:
            ...
            edge_output_dir = os.path.join(output_dir, ext, attribute)
            os.makedirs(edge_output_dir, exist_ok=True)
            output_file = os.path.join(edge_output_dir, f"edge_{edge_id}_attribute_{attribute}_avg.{ext}")
            plt.savefig(output_file)
...
def generate_summary_2graphs_by_intervals(interval_data, output_dir):
    ...
    for attribute, y_data in y_dict_sum.items():
        ...
        time_formatter = mdates.DateFormatter('%H:%M')
        ax.xaxis.set_major_formatter(time_formatter)
        plt.bar(x_data, y_data, width=timedelta(minutes=5), label='current control')
        rand_scale = 0.1 * np.min(y_data)
        plt.bar(x_data + timedelta(minutes=5), y_data + rand_scale * (3*np.random.rand(*y_data.shape)-1),
                width=timedelta(minutes=5), label='modified control at 6.120')
        plt.title(f"Summary of attribute '{attribute}' for Evropská street")
        plt.ylabel(f"total({LABELS[attribute]})")
        plt.xlabel("Time")
...
def main():
    ...
    output_dir = '../outputs/graphs/traffic'
